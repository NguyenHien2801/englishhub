...
interface Rubric {
  ma_key: string; ten_tieu_chi: string; diem_toi_da: number; mo_ta: string
}
interface BaiLuyenViet {
  id: string; chung_chi: string; cap_do: string; tieu_de: string
  bieu_tuong: string; de_bai: string; so_tu_toi_thieu: number
  so_tu_toi_da: number; thong_tin_ky_thi: string
  rubric_json: Rubric[]; goi_y_json: string[]
}
...
⚠️ QUY TẮC CHẤM BẮT BUỘC:
1. Xác định bài có THỰC SỰ trả lời đúng đề không.
2. Lạc đề/vô nghĩa → tổng điểm KHÔNG ĐƯỢC vượt quá ${Math.floor(task.rubric_json.reduce((s,r)=>s+r.diem_toi_da,0)*0.25)} điểm.
3. Điểm ≥80% chỉ dành cho bài xuất sắc.
4. Chấm dựa trên NỘI DUNG THỰC TẾ.
5. Bài ngắn/thiếu ý → trừ điểm nặng.
THÔNG TIN KỲ THI: ${task.thong_tin_ky_thi}
ĐỀ BÀI: ${task.de_bai}
BÀI LÀM (${wordCount} từ):
${text}
TIÊU CHÍ: ${rubricLines}
...
Trả về JSON thuần:
{
  ...
  "band": "<band>",
  "overview": "<nhận xét 3-4 câu>",
  "strengths": ["<1>","<2>"],
  "improvements": ["<1>","<2>","<3>"],
  "correctedSentence": "<nếu ĐÚNG đề: 1 câu lỗi + sửa. SAI đề: null>",
  "rewriteSuggestion": "<nếu ĐÚNG đề: gợi ý tăng điểm. SAI đề: null>",
  "criteria": [
${criteriaJson}
  ]
}`
...
// ── Small score ring for list cards ──────────────────────────────────────────
    ...
    <div style={{
      ...
      <button onClick={() => setOpen(o => !o)} style={{
        ...
        <div style={{
          ...
          <span style={{ fontSize: 20, fontWeight: 800, color: s.text, lineHeight: 1, fontFamily: "'Playfair Display', serif" }}>{c.score}</span>
          <span style={{ fontSize: 11, color: C.textLt, fontWeight: 500 }}>/{c.max}</span>
        </div>
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
            <span style={{ fontSize: 15, fontWeight: 700, color: C.navy, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', paddingRight: 8 }}>{c.criterion}</span>
            <span style={{ flexShrink: 0, fontSize: 12, fontWeight: 700, padding: '3px 10px', borderRadius: 8, background: s.bg, color: s.text, border: `1px solid ${s.border}` }}>{c.band}</span>
          </div>
          <div style={{ height: 5, background: `${C.navy}08`, borderRadius: 3, overflow: 'hidden' }}>
            <div style={{ height: '100%', width: `${pct}%`, background: s.bar, borderRadius: 3, transition: 'width .7s cubic-bezier(.16,1,.3,1)' }} />
          </div>
        </div>
      ...
      {open && (
        <div style={{ padding: '0 22px 22px', borderTop: `1px solid ${C.border}` }}>
          {c.examples && (
            <div style={{ marginTop: 16, padding: '12px 16px', background: 'rgba(100,120,240,.05)', borderLeft: `3px solid ${C.violet}`, borderRadius: '0 12px 12px 0' }}>
...
export default function WritingPage() {
  ...
  useEffect(() => {
    ...
    // Load daLamMap from writing history API if available
    fetch('/api/writing?history=1')
      .then(r => r.json())
  ...
  async function submitWriting() {
    ...
    if (wordCount < selected.so_tu_toi_thieu) {
      toast.error(`Cần tối thiểu ${selected.so_tu_toi_thieu} từ (hiện: ${wordCount})`); return
    }
    const viPattern = /[àáạảãâầấậẩẫăằắặẳẵèéẹẻẽêềếệểễìíịỉĩòóọỏõôồốộổỗơờớợởỡùúụủũưừứựửữỳýỵỷỹđ]/i
    if (viPattern.test(text)) { toast.error('Vui lòng viết bài bằng tiếng Anh!'); return }
    const uniqueWords = new Set(text.trim().toLowerCase().split(/\s+/))
    if (uniqueWords.size < 10) { toast.error('Bài viết quá đơn giản, vui lòng viết đầy đủ hơn!'); return }
    setSubmitting(true)
    try {
      ...
      fetch('/api/writing', {
        ...
        setDaLamMap(p => ({
          ...p,
          [selected.id]: {
            diem: parsed.totalScore,
            tong: selected.rubric_json.reduce((s, r) => s + r.diem_toi_da, 0),
  ...
  if (view === 'list') {
    ...
    const highCount = Object.values(daLamMap).filter(v =>
      Math.round(v.diem / v.tong * 100) >= 80
    ).length
      ...
      <div style={{ maxWidth: 1100, margin: '0 auto', paddingTop: 36, paddingBottom: 80, fontFamily: "'DM Sans', sans-serif" }}>
        ...
        <div style={{
          ...
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, padding: '5px 16px', background: 'rgba(201,168,76,.12)', border: '1px solid rgba(201,168,76,.28)', borderRadius: 50, fontSize: 11, fontWeight: 700, color: C.gold, textTransform: 'uppercase' as const, letterSpacing: '1px', marginBottom: 20 }}>
            <PenLine size={11} strokeWidth={2.5} /> Luyện kỹ năng viết
          </div>
          <h1 style={{ fontFamily: "'Playfair Display', serif", fontSize: 'clamp(32px,4.5vw,52px)', fontWeight: 900, color: '#fff', marginBottom: 16, lineHeight: 1.1, letterSpacing: '-0.5px' }}>
            Writing <em style={{ fontStyle: 'italic', color: C.gold }}>AI Grading</em>
          </h1>
                      ...
                      return (
                        <button key={task.id} className="task-card"
                          ...
                          <div style={{ display: 'flex', alignItems: 'flex-start', gap: 16, marginBottom: 16 }}>
                            <div style={{ width: 54, height: 54, borderRadius: 16, flexShrink: 0, background: `${CERT_COLOR[task.chung_chi] || C.slate}10`, border: `1px solid ${CERT_COLOR[task.chung_chi] || C.slate}20`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 26 }}>
                              {task.bieu_tuong}
                            </div>
                            <div style={{ flex: 1, minWidth: 0 }}>
                              ...
                              <div style={{ fontSize: 16, fontWeight: 700, color: C.navy, lineHeight: 1.35 }}>{task.tieu_de}</div>
                            </div>
                          </div>
                          <p style={{ fontSize: 14, color: C.textMid, lineHeight: 1.65, display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden', marginBottom: 16 }}>{task.de_bai}</p>
                          <div style={{ paddingTop: 14, borderTop: `1px solid ${C.border}`, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                            <span style={{ fontSize: 13, color: C.textLt }}>{task.so_tu_toi_thieu}–{task.so_tu_toi_da} từ · {task.rubric_json.length} tiêu chí</span>
                            {/* Đã làm / Chưa làm badge */}
                            {pct !== null ? (
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 360px', gap: 24, alignItems: 'start' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
            <Panel>
              ...
              <div style={{ background: C.bg, borderRadius: 14, padding: '18px 22px', fontSize: 15, color: C.navy, lineHeight: 1.85, borderLeft: `3px solid ${certColor}` }}>{selected.de_bai}</div>
              <div style={{ display: 'flex', gap: 20, marginTop: 14, alignItems: 'center' }}>
                <span style={{ fontSize: 14, color: certColor, fontWeight: 600 }}>📏 {selected.so_tu_toi_thieu}–{selected.so_tu_toi_da} từ</span>
            ...
            <Panel style={{ padding: 0, overflow: 'hidden' }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '16px 22px', borderBottom: `1px solid ${C.border}` }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                  <div style={{ width: 90, height: 5, background: `${C.navy}08`, borderRadius: 3, overflow: 'hidden' }}>
                    <div style={{ height: '100%', borderRadius: 3, width: `${Math.min(100, wordCount / selected.so_tu_toi_da * 100)}%`, background: wordStatus === 'ok' ? C.green : wordStatus === 'high' ? C.gold : `${C.navy}20`, transition: 'width .3s' }} />
                  </div>
                  <span style={{ fontSize: 14, fontWeight: 700, fontFamily: 'monospace', color: wordStatus === 'ok' ? C.green : wordStatus === 'high' ? '#7a5c00' : C.rose }}>
              ...
              <textarea
                ...
                style={{ width: '100%', minHeight: 340, padding: '22px', fontSize: 16, color: C.text, lineHeight: 1.85, background: 'transparent', border: 'none', resize: 'vertical', fontFamily: "'DM Sans', sans-serif" }}
                rows={14}
              />
            </Panel>

            <button className="submit-btn" onClick={submitWriting}
              disabled={submitting || wordCount < selected.so_tu_toi_thieu}
              style={{
                width: '100%', padding: '16px 0',
                background: submitting ? C.textLt : wordCount < selected.so_tu_toi_thieu ? `${C.navy}14` : C.gold,
                color: submitting || wordCount < selected.so_tu_toi_thieu ? C.textLt : C.navy,
                fontWeight: 700, fontSize: 16, border: 'none', borderRadius: 50,
                cursor: submitting || wordCount < selected.so_tu_toi_thieu ? 'not-allowed' : 'pointer',
                fontFamily: "'DM Sans', sans-serif",
                boxShadow: submitting || wordCount < selected.so_tu_toi_thieu ? 'none' : '0 8px 24px rgba(201,168,76,.4)',
                display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 9,
              }}>
              <Send size={16} strokeWidth={2.2} />
              {submitting ? 'Đang chấm bài...' : wordCount < selected.so_tu_toi_thieu ? `Cần thêm ${selected.so_tu_toi_thieu - wordCount} từ nữa` : `Nộp bài (${wordCount})`}
            </button>
          </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
            <div style={{ display: 'flex', background: C.white, borderRadius: 14, border: `1px solid ${C.border}`, padding: 5, boxShadow: '0 2px 10px rgba(15,28,53,.05)' }}>
              {([['editor', '✍️ Viết bài'], ['guide', '💡 Hướng dẫn']] as const).map(([tab, label]) => (
                <button key={tab} className="tab-btn" onClick={() => setActiveTab(tab)} style={{ flex: 1, padding: '9px 0', borderRadius: 10, fontSize: 14, fontWeight: 600, border: 'none', cursor: 'pointer', fontFamily: "'DM Sans', sans-serif", background: activeTab === tab ? C.navy : 'transparent', color: activeTab === tab ? '#fff' : C.textMid }}>{label}</button>
              ))}
            </div>
                ...
                <Panel>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                    {selected.rubric_json.map((r, i) => (
                      <div key={i} style={{ padding: '14px 16px', borderRadius: 14, background: C.bg, border: `1px solid ${C.border}` }}>
            ...
            {activeTab === 'editor' && (
              <Panel style={{ background: C.goldPale, border: `1px solid ${C.borderMd}` }}>
                <div style={{ display: 'flex', gap: 12, alignItems: 'flex-start' }}>
                  ...
                  <div>
                    <div style={{ fontSize: 15, fontWeight: 700, color: '#7a5c00', marginBottom: 6 }}>Lưu ý khi viết bài</div>
                    <div style={{ fontSize: 14, color: '#7a5c00', lineHeight: 1.72 }}>Viết đủ số từ yêu cầu. AI sẽ trích dẫn trực tiếp từ bài làm của bạn để nhận xét — hãy viết tự nhiên, không copy mẫu.</div>
                  </div>
                </div>
              </Panel>
            )}
      ...
      <div style={{ maxWidth: 1100, margin: '0 auto', paddingTop: 36, paddingBottom: 80, fontFamily: "'DM Sans', sans-serif" }}>
        ...
        <div className="fade-in" style={{ background: C.navy, borderRadius: 28, padding: '36px 40px', marginBottom: 28, display: 'flex', alignItems: 'center', gap: 36, position: 'relative', overflow: 'hidden', flexWrap: 'wrap', boxShadow: '0 20px 56px rgba(15,28,53,.22)' }}>
          ...
          <div style={{ flex: 1, minWidth: 200 }}>
            ...
            <div style={{ fontFamily: "'Playfair Display', serif", fontSize: 'clamp(26px,3.5vw,40px)', fontWeight: 900, color: '#fff', marginBottom: 8, letterSpacing: '-0.3px' }}>{feedback.band}</div>
            <div style={{ fontSize: 15, color: 'rgba(255,255,255,.42)', lineHeight: 1.5 }}>{selected.thong_tin_ky_thi}</div>
          </div>
          <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap' }}>
            <button onClick={() => { setText(''); setFeedback(null); setView('write') }} style={{ padding: '11px 24px', borderRadius: 50, background: 'rgba(255,255,255,.08)', border: '1.5px solid rgba(255,255,255,.18)', color: 'rgba(255,255,255,.85)', fontSize: 15, fontWeight: 600, cursor: 'pointer', fontFamily: "'DM Sans', sans-serif", display: 'flex', alignItems: 'center', gap: 8, transition: 'all .25s' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 360px', gap: 24, alignItems: 'start' }}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
            <Panel className="fade-in">
              ...
              <div style={{ padding: '18px 20px', background: C.goldPale, borderRadius: 14, border: `1px solid ${C.borderMd}`, fontSize: 15, color: '#5a4000', lineHeight: 1.85 }}>{feedback.overview}</div>
            </Panel>

            {feedback.correctedSentence && isOnTopic && (
              <Panel>
                <SectionHeader icon={PenLine} title="Ví dụ sửa lỗi" color={C.violet} />
                <div style={{ padding: '16px 18px', background: 'rgba(100,120,240,.05)', borderLeft: `3px solid ${C.violet}`, borderRadius: '0 14px 14px 0', fontSize: 15, color: C.textMid, lineHeight: 1.78 }}>{feedback.correctedSentence}</div>
              </Panel>
            )}
            ...
            {feedback.rewriteSuggestion && isOnTopic && (
              <Panel style={{ background: '#F0EFFE', border: '1px solid rgba(100,120,240,.22)' }}>
                ...
                <div style={{ padding: '16px 18px', background: 'rgba(100,120,240,.07)', borderRadius: 14, border: `1px solid rgba(100,120,240,.15)`, fontSize: 15, color: '#3a3a6a', lineHeight: 1.85 }}>{feedback.rewriteSuggestion}</div>
              </Panel>
            )}
            ...
            <Panel>
              ...
              <div style={{ padding: '18px', background: C.bg, borderRadius: 14, fontSize: 15, color: C.text, lineHeight: 1.85, whiteSpace: 'pre-wrap', maxHeight: 320, overflowY: 'auto', border: `1px solid ${C.border}` }}>{text}</div>
            </Panel>
          </div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                {feedback.criteria.map((c, i) => {
                  ...
                  return (
                    <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                      <div style={{ width: 40, height: 40, borderRadius: 12, flexShrink: 0, background: cs.bg, border: `1px solid ${cs.border}`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 15, fontWeight: 800, color: cs.text, fontFamily: "'Playfair Display', serif" }}>{c.score}</div>
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ fontSize: 13, fontWeight: 600, color: C.navy, marginBottom: 5, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{c.criterion}</div>
                        <div style={{ height: 5, background: `${C.navy}08`, borderRadius: 3, overflow: 'hidden' }}>
                          <div style={{ height: '100%', width: `${Math.round(c.score / c.max * 100)}%`, background: cs.bar, borderRadius: 3, transition: 'width .7s cubic-bezier(.16,1,.3,1)' }} />
                        </div>
                      </div>
                ...
                <div style={{ marginTop: 6, paddingTop: 14, borderTop: `1px solid ${C.border}`, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  ...
                  <span style={{ fontSize: 22, fontWeight: 900, color: s.text, fontFamily: "'Playfair Display', serif" }}>
                    {feedback.totalScore}<span style={{ fontSize: 14, color: C.textLt }}>/{totalMax}</span>
                  </span>
                </div>
              </div>
            </Panel>
            ...
            <Panel style={{ background: '#FEF2F2', border: '1px solid rgba(240,100,100,.22)' }}>
              ...
              {feedback.improvements.map((imp, i) => (
                <div key={i} style={{ display: 'flex', gap: 10, marginBottom: 10, fontSize: 14, color: '#5a1a1a', lineHeight: 1.65 }}>
                  <ArrowRight size={15} color={C.rose} strokeWidth={2} style={{ flexShrink: 0, marginTop: 2 }} />{imp}
                </div>
              ))}
            </Panel>
          </div>
