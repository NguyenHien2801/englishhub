...
const ADMIN_NAV: NavItem[] = [
  ...
  },
  {
    ...
    children: [
      ...
      { href: '/admin/exams/history',   label: 'Lịch sử luyện kỹ năng' },
...
// ─── CSS — đồng bộ hoàn toàn với Navbar ──────────────────
// Triết lý: cùng font stack (DM Sans), cùng màu sắc (#0F1C35 navy, #C9A84C gold),
// không letter-spacing quá cao, không Playfair Display lung tung, không decoration thừa.
const CSS = `
...
.as-logo-img{
  ...
  box-shadow:0 3px 14px rgba(15,28,53,0.35),0 0 0 1px rgba(201,168,76,0.25);
...
.as-toggle{
  ...
  background:#1a2840;border:1px solid rgba(201,168,76,0.25);
  cursor:pointer;display:flex;align-items:center;justify-content:center;
  color:rgba(201,168,76,0.6);z-index:10;flex-shrink:0;
...
/* ── Section label — nhỏ gọn, không la hét ── */
.as-section-label{
...
/* ── Nav item — font size khớp Navbar (15px → 14px cho sidebar) ── */
.as-item{
...
.as-item::after{
  ...
  background:#C9A84C;transition:height 0.18s cubic-bezier(0.16,1,0.3,1);
}
.as-item:hover{
...
.as-child{
  ...
  padding:8px 16px 8px 42px;margin:1px 8px;border-radius:8px;
...
/* Avatar dùng cùng style với Navbar — white bg, gold border */
.as-user-avatar{
  width:34px;height:34px;border-radius:9px;flex-shrink:0;
  background:#fff;
  border:2px solid rgba(201,168,76,0.5);
  display:flex;align-items:center;justify-content:center;
  font-family:'DM Sans',sans-serif;font-size:14px;font-weight:600;color:#0F1C35;
  overflow:hidden;
}
  ...
  return (
    <aside className="as-wrap" style={{ width: w }}>
        ...
        <div className="as-logo-img">
          <img
            ...
            onError={e => {
              ...
              fallback.style.cssText = 'font-family:DM Sans,sans-serif;font-size:15px;font-weight:700;color:#0F1C35;'
              t.parentElement?.appendChild(fallback)
            }}
          />
        </div>
        {!isCollapsed && (
          <div className="as-logo-label">
            <div className="as-logo-title">EnglishHub</div>
          </div>
        )}
      </Link>

      {/* ── Collapse toggle ── */}
      <button className="as-toggle" onClick={() => setIsCollapsed(v => !v)}
      ...
      <div className="as-footer">
        ...
        {/* User info — avatar style khớp với NavAvatar trong Navbar */}
        {!isCollapsed && (
        ...
        <Link
          href="/dashboard"
          className="as-foot-btn"
          title={isCollapsed ? 'Về giao diện sinh viên' : undefined}
        >
          {Icons.back}
